...
var MarkdownHighlightRules = function() {
    ...
    this.addRules({
        "basic" : [ {
            ...
            token: ["constant"],
            regex: "(\\!\\[flag-" + escaped("]") + "\\]\\(" + escaped(')') + "\\))" 
            

        }, { // link by url
            token : function(...e){
                const c = e[3].trim();
                const h = (c.startsWith('#') && window.$bookIndex.chapters.has(c.substring(1).trim())) ?  "markup.link" : "markup.link.brokenlink";
                return ["text", "string", "text", h, "string", "text"]
            },
            regex : "(\\!?\\[)(" +                                        // [
            ...
            token : function(...e){
                ...
                const p = $bookIndex.properties
                if(p.disableShortLinks && p.disableShortLinks.trim() == "true") return ["string"];
                return  [window.$bookIndex.chapters.has(c.trim()) ?  "markup.link" : "markup.link.brokenlink"]
            },
            regex : "(\\[" +                                      // [
                escaped("]") +                                    // link text 
                "\\])"                                            // ]
